    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg
          ...
          stroke={s <= Math.round(value) ? "#F59E0B" : "#3F3F46"}
          strokeWidth="1.5">
...
function SettingsDrawer({
  ...
  const items = [
    {
      ...
      onClick: () => {
        onClose();
        router.push("/dashboard/shop/profile/settings/edit");
      },
    },
    {
      label: "Notifications",
  ...
  return (
    <div
      ...
      aria-label="Settings">
      <div
        className="w-full max-w-md rounded-3xl border border-white/[0.08]
...
// Moved here from _shop-dashboard.tsx — same component, unchanged. Mirrors
// HomeLocationCard's pattern: always shows the current saved address (not a
// blank "share" prompt with no memory of prior state), and the update
// itself resolves + saves a real address string, not just coordinates.
...
function ShareShopLocationButton({
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        startTransition(async () => {
          ...
          try {
            await shareShopLocation(lat, lng, resolvedAddress);
  ...
  return (
    <div className="mb-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4 space-y-2.5">
      <div>
        <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide">
          Shop Location
        </p>
        <p className="text-sm text-zinc-300 mt-1">{address}</p>
      </div>
      {error && (
      ...
      <button
        ...
        {locating || isPending ? "Getting location…" : "Update shop location"}
      </button>
    </div>
  );
}
...
export default function ShopProfileView(props: ShopProfileProps) {
  const router = useRouter();
  ...
  return (
    <div className="min-h-screen w-full bg-[#080909] relative">
      ...
      <div
        ...
        <div
          className="absolute -top-40 left-1/2 -translate-x-1/2 w-[700px] h-[450px]
      ...
      <div className="relative z-10 w-full p-4 pb-28">
        ...
        <div className="flex flex-col items-center mb-6">
          <div className="relative mb-3">
            {props.logoUrl ? (
              ...
              <div
                className="w-24 h-24 rounded-full bg-amber-400/10 border-2 border-amber-400/30
                flex items-center justify-center">
          ...
          <h1 className="text-xl font-black text-zinc-100 mb-1">
            {props.name}
          </h1>
        ...
        <div className="mb-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-sm text-zinc-400">{props.address}</span>
            </div>
            {props.email && (
              <div className="flex items-center gap-2.5">
                <svg
            ...
            {props.phone && (
              <div className="flex items-center gap-2.5">
                <svg
                  ...
                  aria-hidden="true">
                  <path
                    d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.63 3.4 2 2 0 0 1 3.6 1.22h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.9a16 16 0 0 0 6 6l.95-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"
                    stroke="#71717A"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                  />
        ...
        {/* Services — mirrors the mechanic profile's Specializations block,
            RepairShop.services is a plain String[] (not a coded enum like
            mechanic specialization), so these render as-is, no label map. */}
        <div className="mb-4 rounded-2xl border border-white/[0.08] bg-white/[0.03] p-4">
          <p className="text-xs font-semibold text-zinc-500 uppercase tracking-wide mb-3">
            Services
          </p>
          <div className="flex flex-wrap gap-2">
            {props.services.length > 0 ? (
              ...
              <p className="text-xs text-zinc-600">No services listed</p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5">
          <button
            onClick={() => router.push("/dashboard/shop/jobs")}
            ...
            <div>
              <p className="text-xs font-semibold text-zinc-200">Shop Jobs</p>
              <p className="text-[10px] text-zinc-600">View all bookings</p>
            </div>
          </button>
          <button
            onClick={() => router.push("/dashboard/shop/mechanics")}
            className="flex items-center gap-2.5 px-4 py-3.5 rounded-2xl
              bg-white/[0.03] border border-white/[0.08]
              hover:bg-white/[0.06] transition-colors text-left">
            <div
              className="w-8 h-8 rounded-xl bg-emerald-400/10 border border-emerald-400/20
              flex items-center justify-center shrink-0">
              <svg
                width="14"
                height="14"
                ...
                aria-hidden="true">
                <path
                  d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2 M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8z"
                  stroke="#34D399"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                />
            ...
            <div>
              <p className="text-xs font-semibold text-zinc-200">Roster</p>
              <p className="text-[10px] text-zinc-600">Manage mechanics</p>
            </div>
          </button>
        </div>
      </div>
